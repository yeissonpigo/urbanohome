...
def create_venta(user_id, total, request):
    ...
    if len(Venta.objects.filter(clienteId = cliente)) == 0:
        #For real time, delete id field so it is autogenerated by system
        venta = Venta(id = 12515319,clienteId=cliente, fecha=date.today(), total=total, estadoId=Estado.objects.get(id=1), direccion='No aplica')
        carros = Carro.objects.filter(clienteId = cliente)
        total = get_total(cliente)
        new_pedido = 0
